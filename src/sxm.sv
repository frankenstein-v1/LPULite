...
module sxm (
    ...
    // The 24-bit Control Word (12 bits for Data router, 12 bits for Weight router)
    // Every 3 bits controls a single output lane's multiplexer
    input  logic [11:0] opcode_data,
    input  logic [11:0] opcode_weight,
    
    // Incoming Streams
    input  superlane_t stream_in_west,  // Data moving East
    ...
    superlane_t data_d1, data_d2, data_d3;
    superlane_t weight_d1, weight_d2, weight_d3;

    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            data_d1 <= '0; data_d2 <= '0; data_d3 <= '0;
            weight_d1 <= '0; weight_d2 <= '0; weight_d3 <= '0;
        end else begin
            // Data shifts one box to the right every clock cycle
            data_d1 <= stream_in_west;
            data_d2 <= data_d1;
            data_d3 <= data_d2;
    ...
    always_comb begin
        ...
        for (int i = 0; i < 4; i++) begin
            ...
            logic [2:0] ctrl_data   = opcode_data[i*3 +: 3];
            logic [2:0] ctrl_weight = opcode_weight[i*3 +: 3];
            
            // --- EASTBOUND ROUTER (DATA) ---
            case (ctrl_data)
                ...
                // Staggering (Delays)
                3'b100: stream_out_to_mxm_left[i*8 +: 8] = data_d1[i*8 +: 8];
                3'b101: stream_out_to_mxm_left[i*8 +: 8] = data_d2[i*8 +: 8];
                3'b110: stream_out_to_mxm_left[i*8 +: 8] = data_d3[i*8 +: 8];
                // Bubbles
