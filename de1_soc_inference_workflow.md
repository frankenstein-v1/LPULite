...
The FP32 float parameters stored inside `tiny_lm_model.pt` must be converted into the quantized 8-bit formats required by the LPU's hardware execution units. Run this python script on your PC:
...
checkpoint = torch.load("tiny_lm_model.pt", map_location="cpu")
