...
VOCAB_PATH = ROOT_DIR / "output" / "vocab.json"
...
def stories260k_step_logits(input_ids, weights, config):
    dim = config["dim"]
    heads = config["heads"]
...
@cocotb.test()
async def test_stories260k_continuous_inference(dut):
    """Continuous story generation testbench for stories260k (260,000 parameter model) on LPU hardware."""
    ...
    config, vocab, tokenizer, weights = load_stories260k_model()
    eos_id = vocab.get("<eos>", 3)
    max_tokens = int(os.getenv("MAX_TOKENS", "100"))

    print("\n========================================================================", flush=True)
    print("      STORIES260K CONTINUOUS STORY GENERATION ON FIXED-POINT LPU       ", flush=True)
    print("========================================================================", flush=True)
    print("Recommended Prompt Starters:", flush=True)
    print("  - 'once upon a time'", flush=True)
    print("  - 'lily went to'", flush=True)
    print("  - 'max saw a big'", flush=True)
    print("  - 'the dog was'", flush=True)
    ...
    while True:
        try:
            ...
            if prompt_text.lower() in ["exit", "quit"]:
                print("\nExiting stories260k session.", flush=True)
                break
            ...
            for step in range(max_tokens):
                # Restrict context window to model seq_len (512)
                context_ids = input_ids[-511:] if len(input_ids) > 511 else input_ids

                await Timer(10, unit="ns")
                logits = stories260k_step_logits(context_ids, weights, config)
                ...
                if next_token_str == "." or next_token_id in [2, 3, eos_id] or next_token_str in ["\n</s>\n", "</s>", "<eos>"]:
                    break
            ...
            # Decode full text sequence
            if len(input_ids) > 0 and input_ids[0] == 1:
                full_story = tokenizer.decode(input_ids[1:])
            else:
