...
def run_stories260k_inference():
    src_dir = (Path(__file__).parent / "../src").resolve()
    tb_dir = Path(__file__).parent.resolve()
    sim = os.getenv("SIM", "icarus")
    build_dir = (tb_dir / f"sim_build_stories260k_{sim}").resolve()
    ...
    runner.test(
        hdl_toplevel="lpu_cocotb_top",
        test_module="stories260k_tb",
        testcase="test_stories260k_continuous_inference",
        build_dir=build_dir,
        waves=False,
    )

if __name__ == "__main__":
    run_stories260k_inference()
