...
def run_tiny_lm_inference():
    root_dir = Path(__file__).resolve().parents[2]
    src_dir = root_dir / "src"
    tb_dir = Path(__file__).parent.resolve()
    sim = os.getenv("SIM", "icarus")
    build_dir = (tb_dir / f"sim_build_tiny_lm_{sim}").resolve()
    ...
    runner.test(
        hdl_toplevel="lpu_cocotb_top",
        test_module="tiny_lm_tb",
        testcase="test_continuous_interactive_tiny_lm_inference",
        build_dir=build_dir,
        waves=False,
    )

if __name__ == "__main__":
    run_tiny_lm_inference()
