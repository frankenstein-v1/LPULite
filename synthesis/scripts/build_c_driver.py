...
DRIVER_DIR = ROOT_DIR / "synthesis" / "driver"
INCLUDE_DIR = DRIVER_DIR / "include"
OUTPUT_EXE = DRIVER_DIR / ("lpu_driver.exe" if sys.platform == "win32" else "lpu_driver")

def generate_headers():
    print("  [1/2] Exporting C header files...", flush=True)
    cmd = [sys.executable, str(ROOT_DIR / "model" / "tools" / "export_c_headers.py")]
...
def compile_c_driver():
    ...
    if comp_type in ("gcc", "clang"):
        cmd1 = [comp_path, "-O3", "-Wall", f"-I{INCLUDE_DIR}", str(DRIVER_DIR / "lpu_driver.c"), "-o", str(OUTPUT_EXE)]
        cmd2 = [comp_path, "-O3", "-Wall", f"-I{INCLUDE_DIR}", str(DRIVER_DIR / "lpu_jtag_runner.c"), "-o", str(jtag_exe)]
    else: # MSVC cl
        cmd1 = [comp_path, "/O2", f"/I{INCLUDE_DIR}", str(DRIVER_DIR / "lpu_driver.c"), f"/Fe:{OUTPUT_EXE}"]
        cmd2 = [comp_path, "/O2", f"/I{INCLUDE_DIR}", str(DRIVER_DIR / "lpu_jtag_runner.c"), f"/Fe:{jtag_exe}"]
