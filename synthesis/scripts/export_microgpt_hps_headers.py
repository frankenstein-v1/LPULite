...
talks to the LPULite over the DE1-SoC HPS-to-FPGA lightweight bridge.
"""
...
def display_path(path: Path) -> str:
    """Return a reproducible repository-relative path when possible."""
    try:
...
def c_array_rows(name: str, rows: list[int]) -> str:
    lines = [f"static const lpulite_row96_t {name}[] = {{"]
    for value in rows:
 ...
 * Source artifacts: {display_path(artifact_dir)}
